...
from ai_media_generation.domain.anima.lora_dataset.subject import (
    Subject,
    SubjectFeature,
...
class SubjectRepository:
    def find(self) -> tuple[Subject, ...]:
        directory = Config().anima_lora_training_characters_directory
        subjects: list[Subject] = []
        names: dict[str, Path] = {}
        for path in self._json_paths(directory):
            subject = self._to_subject(
                read_json(path, ANIMA_LORA_SCHEMAS["characters"])
            )
